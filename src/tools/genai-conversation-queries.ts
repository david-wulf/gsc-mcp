...
export async function genaiConversationQueries(
  days: number = 480,
  ...
  const byQuery = new Map<
    string,
    { impressions: number; clicks: number; posWeight: number; pages: Map<string, number> }
  >();
  for (const r of rows) {
  ...
  let timeline: Array<{ month: string; impressions: number; clicks: number }> | undefined;
  if (includeTimeline) {
