...
server.registerTool("quick_wins", {
    description: "Find keywords you're almost ranking for that could be pushed to page one. Returns queries at positions 4-15 with high impressions, sorted by traffic opportunity." + guardrails_js_1.GUARDRAIL_SUFFIX + guardrails_js_1.VISUAL_SUFFIX,
    inputSchema: {
...
server.registerTool("ctr_opportunities", {
    description: "Find pages with high impressions but CTR significantly below expected for their position. These are title/meta description optimisation candidates." + guardrails_js_1.GUARDRAIL_SUFFIX + guardrails_js_1.VISUAL_SUFFIX,
    inputSchema: {
        days: zod_1.z.number().default(28).describe("Number of days to analyse"),
...
server.registerTool("site_snapshot", {
    description: "Get a quick overview of how the site is performing. Returns total clicks, impressions, CTR, and position with a comparison to the prior period." + guardrails_js_1.GUARDRAIL_SUFFIX + guardrails_js_1.VISUAL_SUFFIX,
    inputSchema: {
...
server.registerTool("cannibalization_check", {
    description: "Find keywords where multiple pages from your site compete against each other. Shows which page ranks higher, the position gap, and combined impressions being split." + guardrails_js_1.GUARDRAIL_SUFFIX + guardrails_js_1.VISUAL_SUFFIX,
    inputSchema: {
        days: zod_1.z.number().default(28).describe("Number of days to analyse"),
...
server.registerTool("content_decay", {
    description: "Find pages that are slowly dying with consistent traffic decline over three consecutive 30-day periods. One bad month is noise; three consecutive bad months is a problem." + guardrails_js_1.GUARDRAIL_SUFFIX + guardrails_js_1.VISUAL_SUFFIX,
    inputSchema: {
        surface: surfaceParam("Surface to query: web (default), image, video, news, discover. Discover is page-based and supported."),
    },
}, async ({ surface }) => {
...
server.registerTool("topic_cluster_performance", {
    description: "See how a group of pages performs as a whole. Aggregates clicks, impressions, CTR, and position for all pages matching a URL path pattern, plus top 5 pages and queries." + guardrails_js_1.GUARDRAIL_SUFFIX + guardrails_js_1.VISUAL_SUFFIX,
    inputSchema: {
...
server.registerTool("ctr_vs_benchmark", {
    description: "Compare your actual CTR per page against industry benchmarks by position. Flags pages significantly underperforming for their ranking position." + guardrails_js_1.GUARDRAIL_SUFFIX + guardrails_js_1.VISUAL_SUFFIX,
    inputSchema: {
        days: zod_1.z.number().default(28).describe("Number of days to analyse"),
...
server.registerTool("advanced_search_analytics", {
    description: "Run a custom search analytics query with flexible dimensions and filters. Supports country, device, query, and page filtering. For power users who need specific data cuts." + guardrails_js_1.GUARDRAIL_SUFFIX + guardrails_js_1.VISUAL_SUFFIX,
    inputSchema: {
        days: zod_1.z.number().default(28).describe("Number of days to analyse"),
...
server.registerTool("multi_site_dashboard", {
    description: "Health check across multiple GSC properties in one view. Shows clicks, impressions, CTR, and position for each site with period comparison and health status. Agency essential." + guardrails_js_1.GUARDRAIL_SUFFIX + guardrails_js_1.VISUAL_SUFFIX,
    inputSchema: {
